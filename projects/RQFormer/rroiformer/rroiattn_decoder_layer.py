...
class RRoIAttnDecoderLayer(BBoxHead):
    r"""Dynamic Instance Interactive Head:
    Replace Dynamic Convolution to RRoIAttention
    ...
    def __init__(self,
        ...
        assert self.reg_class_agnostic, 'Decoder layer only ' \
            'suppport `reg_class_agnostic=True` '
        assert self.reg_decoded_bbox, 'Decoder layer only ' \
    ...
    def forward(self, roi_feat: Tensor, query: Tensor, query_pos: Tensor) -> tuple:
        ...
        query = self.self_attn_norm(self.self_attn(query))
        self_attn_feats = query.permute(1, 0, 2)                # (bs, num_queries, embed_dims)

        # rroi attention
        query = self_attn_feats                                 # (bs, num_queries, embed_dims)
        rroi_attn = self.rroi_attn(query, roi_feat)
        query = self.rroi_attn_norm(query + self.rroi_attn_dropout(rroi_attn))

        # FFN
        query = self.ffn_norm(self.ffn(query))
        ...
        return cls_score, bbox_delta, query, self_attn_feats

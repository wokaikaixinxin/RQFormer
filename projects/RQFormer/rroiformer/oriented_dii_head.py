...
class OrientedDIIHead(BBoxHead):
    ...
    def forward(self, roi_feat: Tensor, query: Tensor, query_pos: Tensor) -> tuple:
        ...
        proposal_feat = query + query_pos   # bs, num_query, 256

        N, num_proposals = proposal_feat.shape[:2]

        # Self attention
        proposal_feat = proposal_feat.permute(1, 0, 2)  # num_query, bs, 256
        proposal_feat = self.attention_norm(self.attention(proposal_feat))  # num_query, bs, 256
        attn_feats = proposal_feat.permute(1, 0, 2) # bs, num_query, 256

        # instance interactive
        proposal_feat = attn_feats.reshape(-1, self.in_channels)    # bs*num_query, 256
        proposal_feat_iic = self.instance_interactive_conv(
            proposal_feat, roi_feat)                                # bs*num_query, 256
        proposal_feat = proposal_feat + self.instance_interactive_conv_dropout(
            proposal_feat_iic)                                      # bs*num_query, 256
        obj_feat = self.instance_interactive_conv_norm(proposal_feat)   # bs*num_query, 256

        # FFN
        obj_feat = self.ffn_norm(self.ffn(obj_feat))                # bs*num_query, 256
